...
There are two instruction concepts:

1. **Routine instructions** are embedded inside the structured routine and describe how to follow the schedule.
2. **Recommendation instructions** are the top-level `TreatmentRecommendation.instructions` field. If staff do not provide them, the selected protocol's instructions are copied as the default.
...
```typescript
INTERVAL | TIMES_PER_DAY | SPECIFIC_DAYS | SESSION_PLAN | AS_NEEDED
```
...
Valid duration/frequency units are:
...
```json
{
  ...
  "instructions": "Use consistently during the treatment period."
}
```
...
- `instructions`: optional routine instructions.

### `TIMES_PER_DAY`

Used for medication, supplement, or topical daily schedules.

```json
{
  "type": "TIMES_PER_DAY",
  "timesPerDay": 2,
  "timings": ["MORNING_ANYTIME", "EVENING_ANYTIME"],
  "quantity": "1 tablet",
...
- `timesPerDay`: positive integer, maximum 24.
- `timings`: one or more time-of-day values.
- `quantity`: optional dosage or quantity text.
- `duration`: optional total treatment duration.
- `instructions`: optional routine instructions.

The frontend presents timing checkboxes such as morning, before breakfast, after lunch, evening, after dinner, and bedtime.
...
- `frequency`: optional repeating cycle.
- `duration`: optional total treatment duration.
- `instructions`: optional routine instructions.

The frontend presents weekday checkboxes and an optional repeat-frequency control.
...
- `frequency.unit`: day, week, month, or year.
...
- `reason`: optional explanation for as-needed use.
...
- Valid day, timing, duration, and frequency values.
- At least one timing for `TIMES_PER_DAY`.
- At least one day for `SPECIFIC_DAYS`.
- Required frequency and total sessions for `SESSION_PLAN`.
- Maximum `timesPerDay` of 24.
- Maximum routine instruction length of 5,000 characters.
