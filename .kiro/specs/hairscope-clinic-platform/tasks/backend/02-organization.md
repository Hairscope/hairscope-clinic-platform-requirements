...
- [x] 7.1–7.4 Implement the **clinic-level** per-staff visibility (`recordVisibilityMode` `OPEN`/`RESTRICTED`, assignment-based; ClinicAdmins bypass) with unit tests
- [x] 7.5 Write property test: RESTRICTED limits non-admins to assigned records
...
- [x] 10.1 Create `clinic-closure.schema.ts` (date, reason, recurring, status; unique `{ clinicId, date }`)
- [x] 10.2 Repository + service (used by slot availability to block closed dates)
