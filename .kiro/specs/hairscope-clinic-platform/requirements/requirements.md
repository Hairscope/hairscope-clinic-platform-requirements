...
| GI-6 | A Patient record is scoped to the Clinic where it was created. However, the same physical person may have Patient records at multiple Clinics across different Organizations. A future Patient app will aggregate the full cross-clinic journey for the patient. Uniqueness constraints (email, phone) are enforced per Clinic only, not globally. |
