...
1. THE Platform SHALL accept the image and PDF types defined in `shared/api-contracts.md` Section 8 (all common modern image formats — including iPhone HEIC/HEIF — plus `application/pdf`) as medical document uploads via the `uploadFile` GraphQL mutation.
