...
- **MiscCharge**: An additional charge added by Staff to an Invoice that is not associated with a predefined Service or Product.
...
4. WHEN an Invoice is generated, THE Platform SHALL emit `InvoiceGenerated` and record the generation in the AuditLog.
...
1. THE Invoice SHALL include: patient details (name, contact), clinic details (name, address, contact), invoice date, itemized Services, itemized Products, itemized MiscCharges, itemized Discounts (description and amount), Subtotal, Discount Amount, Taxable Amount, Tax (amount and rate), and Total.
