...
| ID&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | Invariant | Enforced By |
|:------------|-----------|-------------|
| GI-25 | AuditLog entries are append-only and cannot be edited, deleted, or soft-deleted. | No update/delete mutation exposed; DB append-only policy |
| GI-26 | AuditLog entries are never transferred when a Staff member is deleted. | Data transfer logic |
| GI-27 | AuditLog entries retain the original actor name even after the actor is deleted. | Snapshot at write time |
| GI-28 | AuditLog entries are retained for a minimum of 7 years. | Retention policy enforcement |
