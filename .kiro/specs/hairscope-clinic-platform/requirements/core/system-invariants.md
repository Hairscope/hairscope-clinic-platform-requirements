...
| GI-6 | A Patient record is scoped to the Clinic where it was created. The same physical person may have Patient records at multiple Clinics (including across different Organizations). Email and phone uniqueness is enforced per Clinic only, not globally. A future Patient app will aggregate the full cross-clinic journey. | DB unique constraint (per clinic) |
