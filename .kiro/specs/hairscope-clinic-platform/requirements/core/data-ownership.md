...
- **Attribution** - who created or performed an action on a record. This is immutable and is captured in the `createdBy`, `authoredBy`, `uploadedBy` fields and in AuditLog entries. Attribution is never modified under any circumstance, including Staff deletion.
...
| Clinic | `createdBy` - the OrganizationAdmin who created it | - | Organization | No |
| ClinicProfile | `lastUpdatedBy` - the Admin who last saved it | - | Clinic | No (part of Clinic record) |
| Staff | `invitedBy` - the Admin who sent the invite | - | Clinic or Org | No (marked INACTIVE, not deleted) |
| Role | `createdBy` - the ClinicAdmin who created it | - | Clinic | No |
...
| TrichoscopyImage | `createdBy` - the Staff member who captured it | - | Clinic | No (follows Session) |
| GlobalImage | `createdBy` - the Staff member who captured it | - | Clinic | No (follows Session) |
| MedicalDocument | `uploadedBy` - the Staff member who uploaded it | - | Clinic | No (document belongs to patient record) |
...
| Appointment | `createdBy` - the Staff member or web component that booked it | `assignedTo` (assigned staff via SmartScheduling) | Clinic | Yes - `assignedTo` reassigned to recipient |
| Invoice | `createdBy` - auto-generated by the system on session completion | - | Clinic | No (invoice belongs to session/patient) |
| Product | `createdBy` - the ClinicAdmin who added it | - | Clinic | No |
| Report | `generatedBy` - auto-generated by the system on session completion | - | Clinic | No (follows Session) |
| WebhookSource config | `createdBy` - the ClinicAdmin who configured it | - | Clinic | No |
| AuditLog entry | `actorId` / `actorName` - the Staff member who performed the action | - | Organization | Never reassigned - immutable |
...
2. Every clinical record (Patient, Session, Lead, Appointment, Invoice, Product, MedicalDocument) SHALL be associated with exactly one Clinic via its `clinicId` field.
...
| Record Type | Rule |
|-------------|------|
| AuditLog entries | Immutable - `actorName` remains the original staff name permanently |
...
5. WHEN reassignment is complete, THE Platform SHALL record the deletion and all recipient assignments in the AuditLog.
...
- AuditLog entries attributed to the deleted Staff member SHALL continue to reference their original name.
...
3. WHEN a Session is deleted, all associated TrichoscopyImages, GlobalImages, annotations, questionnaire answers, and doctor's notes are permanently deleted.
